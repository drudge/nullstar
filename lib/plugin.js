...
Plugin.prototype.handle = function(from, channel, message) {
  ...
  if (matches && matches.length >= 2) {
    ...
    if(this.get('irc channels', ['#nullstars']).indexOf(channel) !== -1) {
      this.emit('handle', cmd, from, args);
      this['$' + cmd].call(this, from, channel, args);
      return true;
    }
  }
...
/**
 * Register any events the plugin should receive.
 *
 * @return {Bot} this
 * @api public
 */
