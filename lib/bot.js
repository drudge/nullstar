...
Bot.prototype.notice = function(channel, msg) {
  debug('noticing %s: %s', channel, msg);
  return this.connection.notice(channel, msg);
};

Bot.prototype.noticeAll = function(channel, msg) {
  // TODO: this shouldn't be here.. need to modularize it
  if (this.get('slack enabled')) {
    debug('should notice slack');

    var slack = (this.plugins.slack || {}).slack;
    if (slack) {
      debug('noticing in slack');
      var chans = slack.channels || {};
      for (var id in chans) {
        var tmpChan = channel.replace(/^#/, '');
        var currentChan = chans[id];
        //debug('id: %s | channel: %s | tmp: %s', id, channel, tmpChan);
        if (typeof currentChan === 'object' && currentChan.name === tmpChan) {
          debug('found');
          var dest = slack.getChannelGroupOrDMByID(id);
  ...
  return this.notice(channel, msg);
}
