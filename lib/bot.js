...
function Bot(opts) {
  ...
  this.db = new SQLite(this.get('database path', __dirname + '/../nullstar.db'));
}
...
Bot.prototype.connect = function(fn) {
  ...
  var irc = this.connection = new IrcClient(self.get('irc server', '127.0.0.1'), self.get('irc nick', 'nullstar'), {
    ...
    autoConnect: false,
    floodProtection: self.get('irc enable flood protection', false),
  ...
  debug('connecting to ' + self.get('irc server', '127.0.0.1') + ':' + self.get('irc port', 6667));
...
/**
 * Notify plugins to register any events they wish to receive.
 *
 * @return {Bot} this
 * @api public
 */

Bot.prototype.registerEvents = function() {
  for(var name in this.plugins) {
    if (this.plugins.hasOwnProperty(name)) {
      var plugin = this.plugins[name];
      if (plugin instanceof Plugin) {
        debug('calling registerEvents on %s', plugin.name);
