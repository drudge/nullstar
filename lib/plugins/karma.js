    ...
    if (~nicks.indexOf(nick)) {
      if (from.toLowerCase() == nick) {
        ...
        this.store(nick, channel, action == '-' ? -1 : 1, function(score) {
          ...
          debug(msg);
          this.bot.noticeAll(channel, msg);
        }.bind(this));
      }
    } else {
...
KarmaPlugin.prototype.$karma = function(from, channel, nick) {
  ...
  storage.fetchOne(this.collectionName, null, ['value'], 'nick = ? and channel = ?', [nick, channel], function(err, karma) {
...
KarmaPlugin.prototype.stats = function(from, channel) {
  this.bot.storage.fetch(this.collectionName, null, null, 'channel = ?', [channel], function(err, users) {
    ...
    if (users.length >= 6) {
      ...
      var msg = noticeAll('bottom 3: %s', tmp.join(' | '));
