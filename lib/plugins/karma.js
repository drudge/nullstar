...
KarmaPlugin.prototype.version = '0.1.0';
...
KarmaPlugin.prototype.handle = function(from, channel, message) {
  ...
  if (matches && matches.length >= 3) {
    ...
    if ('undefined' !== typeof this.bot.connection.chans[channel].users[nick]) {
      if (from == nick) {
...
KarmaPlugin.prototype.$karma = function(from, channel, nick) {
  ...
  db.selectOne(
    ...
    function(err, user) {
      ...
      console.log(self.bot.connection.chans[channel].users);
      if ('undefined' != typeof self.bot.connection.chans[channel].users[nick]) {
