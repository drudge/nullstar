...
InstagramPlugin.prototype.handle = function(id, from, channel, message) {
  // only perform tweet actions on allowed channels
  if (this.get('instagram allowed channels', []).indexOf(channel) === -1) return false;

  // handle plugin commands
  var handled = InstagramPlugin.super_.prototype.handle.call(this, id, from, channel, message);
  ...
  if (!handled && this.get('instagram notice grams', true) && matches && matches.length) {
    ...
    request(reqUrl, function(err, res, body) {
      ...
      if (msg) this.bot.notice(id, channel, msg);
    }.bind(this));
