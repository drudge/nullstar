...
TweetPlugin.prototype.handle = function(from, channel, message) {
  if (!this.get('twitter notice tweets', true)) return;
  ...
  if (matches && matches.length >= 2) {
    ...
    this.twitter.get('/statuses/show/' + id + '.json', {include_entities:true}, function(data) {
      if (!data) return;
      data.user = data.user || {};
      var msg = format('\00310,01\002Twitter\017 | %s (@%s) %s: %s', data.user.name,  data.user.screen_name, timeago(data.created_at), data.text);
      debug(msg);
