...
function TweetPlugin(bot) {
  ...
  this.twitter = new Twitter({
    consumer_key: bot.get('twitter consumer key'),
    consumer_secret: bot.get('twitter consumer secret'),
    access_token_key: bot.get('twitter access token'),
    access_token_secret: bot.get('twitter access token secret')
  });
}
...
TweetPlugin.prototype.$tweet = function(nick, chan, status) {
  ...
  if (status.length) {
    ...
    self.twitter.updateStatus(status, function(data) {
      if (data.id_str) {
        self.bot.connection.notice(chan, format('https://twitter.com/%s/status/%s', data.user.screen_name, data.id_str));
      } else {
...
TweetPlugin.prototype.$rmtweet = function(nick, chan, id) {
  ...
  if (id.length) {
    ...
    self.twitter.destroyStatus(id, function(data) {
      if (data.id_str) {
        self.bot.connection.notice(chan, format('Tweet deleted (%s)', id));
      } else {
        self.error(chan, data);
      }
    });
...
TweetPlugin.prototype.$rmtwat = function(nick, chan, id) {
  return this.$rmtwat(nick, chan, id);
};
