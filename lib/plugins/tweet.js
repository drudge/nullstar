...
TweetPlugin.prototype.$twreply = function(nick, chan, args) {
  ...
  this.twitter.get('/statuses/show/' + id + '.json', {include_entities:true}, function(data) {
    ...
    data.user = data.user || {};
    if (status.length) {
      var isOwnTweet = data.user.screen_name == 'nullstar';
      if (!isOwnTweet && data.user.screen_name && !status.charAt('@')) status = '@' + data.user.screen_name + ' ' + status;
      debug('replying \'%s\' to %s', status, id);
