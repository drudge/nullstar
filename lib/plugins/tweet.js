  ...
  if (this.get('twitter notice tweets', true) && matches && matches.length >= 2) {
    ...
    this.twitter.get('/statuses/show/' + id + '.json', {include_entities:true}, function(data) {
      ...
      var msg = format('\00310,01\002Twitter\017 | %s (@%s) %s: %s',
          data.user.name,  data.user.screen_name, timeago(data.created_at), data.text);
...
TweetPlugin.prototype.$twreply = function(nick, chan, args) {
  ...
  self.twitter.get('/statuses/show/' + id + '.json', {include_entities:true}, function(data) {
    if (!data) return;
    
    data.user = data.user || {};
    if (status.length) {
      var isSomeoneElse = (data.user.screen_name != self.get('twitter username'));
      
      if (isSomeoneElse && data.user.screen_name && !status.charAt(0) != '@') {
        status = '@' + data.user.screen_name + ' ' + status;
      }
      
      debug('replying \'%s\' to %s (%s)', status, id, data.user.screen_name);
      self.twitter.updateStatus(status, { in_reply_to_status_id: id }, function(data) {
        self.emit('tweet', data);
        if (data.id_str) {
          if (self.bot) self.bot.notice(chan, format('https://twitter.com/%s/status/%s', data.user.screen_name, data.id_str));
        } else {
