...
TweetPlugin.prototype.version = '0.5';
...
TweetPlugin.prototype.$retweet = function(nick, chan, id) {
  ...
  if (id.length) {
    ...
    self.twitter.retweetStatus(id, function(data) {
      if (data.id_str) {
        self.bot.notice(chan, format('Retweeted: https://twitter.com/%s/status/%s', data.user.screen_name, data.id_str));
      } else {
        self.error(chan, data);
      }
    });
